...
(function (Handsontable) {
  ...
  function HandsontableFormula() {
    ...
    var beforeAutofillInsidePopulate = function (row, col, value, direction, delta) {
      if (value[0] === '=') {
        return instance.ruleJS.utils.updateFormula(value, direction, delta);
      } else {

        if (delta > 1) {

          var newValue = instance.ruleJS.helper.number(value);

          if (instance.ruleJS.utils.isNumber(newValue)) {
            return instance.ruleJS.utils.updateValue(newValue, direction, delta);
          }
        }
