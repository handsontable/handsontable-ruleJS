...
//      var direction = (selectedRow >= row) ? 'before' : 'after';
      ...
      var items = plugin.matrix.getRefItemsToRow(row),
          counter = 1;
      ...
      items.forEach(function (id) {
        ...
        if (formula !== item.formula) { // formula updated
          ...
          var newId = plugin.utils.changeRowIndex(id, counter),
              cellCoords = plugin.utils.cellCoords(newId);
    ...
    var afterCreateCol = function (col) {
      ...
      var direction = (selectedCol >= col) ? 'before' : 'after';
