...
Handsontable.Core = function (rootElement, userSettings) {
  ...
  grid = {
     ...
     */
    populateFromArray: function (start, input, end, source, method, direction) {
          ...
          for (r = 0; r < rlen; r++) {
            ...
            for (c = 0; c < clen; c++) {
              ...
              if (!instance.getCellMeta(current.row, current.col).readOnly) {
                ...
                if (source === 'autofill') {
                  // TODO: calculate delta for each value in selected range
                  input[r][c] = instance.runHooksAndReturn('beforeAutofillInsidePopulate', current.row, current.col, input[r][c], direction, delta);
                }
   ...
   * @param {String=} [source="populateFromArray"]
   * @param {String=} [method="overwrite"]
   * @param {String} (left|right|up|down)
   * @return {Object|undefined} ending td in pasted area (only if any cell was changed)
   */
  this.populateFromArray = function (row, col, input, endRow, endCol, source, method, direction) {
    if (!(typeof input === 'object' && typeof input[0] === 'object')) {
      throw new Error("populateFromArray parameter `input` must be an array of arrays"); //API changed in 0.9-beta2, let's check if you use it correctly
    }
    return grid.populateFromArray(new WalkontableCellCoords(row, col), input, typeof endRow === 'number' ? new WalkontableCellCoords(endRow, endCol) : null, source, method, direction);
  };
   ...
   */
    Autofill.prototype.apply = function () {
      ...
      if (start) {
        ...
        this.instance.populateFromArray(start.row, start.col, _data, end.row, end.col, 'autofill', null , direction);
